...
const Navbar = () => {
  ...
  const getItemPosition = useCallback((index) => {
    if (index === null || !navRefs.current[index] || !navItemsContainerRef.current) return {};
        ...
        if (position.left !== undefined && position.width !== undefined) {
          ...
          requestAnimationFrame(() => {
            if (indicatorRef.current) {
              indicatorRef.current.style.transitionProperty = 'transform, width, opacity';
              indicatorRef.current.style.transitionDuration = `${INDICATOR_CONFIG.transitionDuration}s`;
              indicatorRef.current.style.transitionTimingFunction = 'cubic-bezier(0.33, 0.83, 0.99, 0.98)';
            }
          });
        }
  ...
  return (
    <div ref={navContainerRef} className='fixed inset-x-0 top-4 h-16 z-50 border-none transition-all duration-700 sm:inset-x-6' aria-label='Main navigation'>
      <header className='absolute top-1/2 w-full -translate-y-1/2'>
        <nav className='flex size-full items-center justify-between p-4'>
          <div className='flex items-center gap-4'>
            ...
            <Button id='whitepaper-button' title='Whitepaper' containerClass={'bg-blue-50 md:flex hidden items-center justify-center gap-1'} />
          </div>

          <div className='flex h-full items-center relative'>
            <div className='hidden md:block'>
              <div ref={navItemsContainerRef} className='relative flex items-center justify-center h-7' onMouseLeave={handleNavMouseLeave}>
                {indicatorVisible && (
                  <div
                    ref={indicatorRef}
                    className='slide-nav'
                    style={{
                      position: 'absolute',
                      left: 0,
                      transform: hoverIndex !== null ? `translateX(${getItemPosition(hoverIndex).left}px)` : 'translateX(0)',
                      width: hoverIndex !== null ? `${getItemPosition(hoverIndex).width}px` : '0',
            ...
            <button
              ...
              <audio ref={audioElementRef} src='/audio/audio_loop.mp3' className='hidden' loop />
              {[1, 2, 3, 4].map((bar) => (
                <div key={bar} className={`indicator-line ${isAudioEnabled ? 'active' : ''}`} style={{ animationDelay: `${bar * 0.1}s` }} />
              ))}
            </button>
          </div>
        </nav>
