...
const Hero = () => {
  ...
  // Helper functions
  const getInactiveVideoRef = useCallback(() => (activeVideoId === 'video-a' ? videoBRef : videoARef), [activeVideoId]);

  const getActiveVideoId = useCallback(() => activeVideoId, [activeVideoId]);

  const getInactiveVideoId = useCallback(() => (activeVideoId === 'video-a' ? 'video-b' : 'video-a'), [activeVideoId]);
  ...
  useEffect(() => {
    ...
    gsap.set(`#${getActiveVideoId()}`, VIDEO_STATES.ACTIVE);
    gsap.set(`#${getInactiveVideoId()}`, VIDEO_STATES.INACTIVE);
  }, [currentIndex, activeVideoId, getActiveVideoId, getInactiveVideoId, getInactiveVideoRef, getVideoSrc, upComingVideoIndex]);
  ...
  const handleMiniVDClick = useCallback(() => {
    ...
    if (inactiveVideoRef.current) {
      inactiveVideoRef.current.src = getVideoSrc(upComingVideoIndex);
      inactiveVideoRef.current.load();
      inactiveVideoRef.current.play().catch((e) => console.warn('Autoplay failed:', e));
    }
  ...
  }, [getActiveVideoId, getInactiveVideoId, getInactiveVideoRef, getVideoSrc, playWhooshSound, setAudio, upComingVideoIndex]);
    ...
    <div className='relative h-dvh w-screen overflow-x-hidden'>
      ...
      <div id='video-frame' className='relative z-10 h-dvh w-screen overflow-hidden rounded-lg bg-blue-75'>
        <div className='relative h-full w-full bg-black'>
          {/* Main videos */}
          <video playsInline ref={videoARef} autoPlay loop muted id='video-a' className='absolute left-0 top-0 size-full object-cover object-center' onLoadedData={handleVideoLoaded} />
          <video playsInline ref={videoBRef} autoPlay loop muted id='video-b' className='absolute left-0 top-0 size-full object-cover object-center' onLoadedData={handleVideoLoaded} />

          {/* Preview video */}
          {previewVisible && (
            <div className='mask-clip-path absolute-center absolute z-50 size-40 overflow-hidden rounded-lg'>
              <VideoPreview>
                <div onClick={handleMiniVDClick} className='preview-video origin-center scale-50 opacity-0 transition-all duration-500 ease-in hover:scale-100 hover:opacity-100 cursor-pointer'>
                  <video
                    playsInline
                    ref={previewVideoRef}
